  ...
  return <main className="min-h-screen bg-background pb-28 sm:pb-16">
    ...
    <div className="mx-auto max-w-7xl px-5 py-7 sm:px-8">
      <div className="grid items-center gap-5 border-b border-border pb-6 sm:grid-cols-[minmax(0,1fr)_auto]">
        <div><p className="text-sm font-bold text-primary">13 captures available</p><p className="mt-1 text-sm text-muted-foreground">Choose a date, then select every wave that is yours.</p></div>
      ...
      <section className="py-7"><div className="mb-4 flex items-baseline gap-3"><h2 className="text-2xl font-semibold text-primary">7:00</h2><span className="text-sm text-muted-foreground">9 captures</span></div><div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">{captures.map((src, i) => <button key={i} className="group relative aspect-[4/3] overflow-hidden rounded-lg border border-border bg-card text-left"><img src={src} alt={`Surf capture at ${7 + Math.floor(i / 4)}:${String(14 + i * 3).padStart(2,"0")}`} width={1024} height={768} loading="lazy" className="h-full w-full object-cover transition-transform duration-300 group-hover:scale-[1.03]"/><span className="absolute right-2 top-2 grid size-7 place-items-center rounded-full bg-primary/90 text-primary-foreground"><Check className="size-4"/></span><span className="absolute bottom-2 left-2 rounded-full bg-card/90 px-2 py-1 text-xs font-bold">07:{String(14 + i * 3).padStart(2,"0")}</span></button>)}</div></section>
    ...
    <div className="fixed inset-x-0 bottom-0 z-40 border-t border-border bg-card p-3 sm:hidden"><Button className="w-full">Book a private session</Button></div>
  </main>;
}